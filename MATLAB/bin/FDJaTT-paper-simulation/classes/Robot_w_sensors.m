...
% position of the robots and targets around him
    ...
    methods
        ...
        function H = partial_measure_model( obj, robot)
            H = zeros(2, 3);
            z1 = robot.true_Pose(1)-obj.true_Pose(1);
            z2 = robot.true_Pose(2)-obj.true_Pose(2);
            H(1,1) = 0.5*(z1^2+z2^2)^(-0.5)*2*z1*(-1);
            H(1,2) = 0.5*(z1^2+z2^2)^(-0.5)*2*z2*(-1);
            H(1,3) = 0;
            H(2,1) = -(1/(1+(self(1)^2/z2)));
            H(2,2) = z1/(z1^2+self(2)^2);
            H(2,3) = -1;
